...
class ElasticSearchJournalistsRepository {
    ...
    async prepareForIndex(fromTo) {
        ...
        if (fromTo) {
            var elements = await Journalist.findAll(fromTo)
        } else {
            var elements = await Journalist.findAll()
        }
...
module.exports = new ElasticSearchJournalistsRepository()
